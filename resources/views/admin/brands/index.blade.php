...
<div class="container">
    <div class="card my-5 rounded-4 shadow">
        ...
        <div class="card-header d-flex justify-content-between align-items-center bg-dark text-white rounded-top-4">
            <h1 class="fw-bold m-0">{{ __('Marcas') }}</h1>
            <a href="{{ route('brands.create') }}" class="btn btn-warning fw-bold">
        ...
        <div class="card-body p-0 overflow-x-auto">
                    ...
                    <tbody>
                        @foreach ($brands as $brand)
                            <tr>
                                ...
                                <td class="px-3">{{ $brand->created_at ? $brand->created_at->format('d/m/Y') : 'No disponible' }}</td>
                                ...
                                <td class="px-3 text-center">
                                    ...
                                    <form action="{{ route('brands.destroy', $brand->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('¿Seguro que deseas eliminar esta marca?')">
            ...
            @else
                <p class="text-center p-4">No hay marcas registradas.</p>
            @endif
        </div>

        <div class="card-footer text-body-secondary text-center">
            Mostrando {{ $brands->count() }} marcas
        </div>
    </div>
</div>
